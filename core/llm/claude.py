...
class ClaudeRouter:
    """Claude 라우팅 Capability - Haiku 모델 사용"""
...
class ClaudeAnswerer:
    """Claude 응답 Capability - Sonnet 모델 사용"""
...
class ClaudeExtractor:
    """Claude 추출 Capability - Haiku 모델 사용"""
...
class ClaudeJudge:
    """Claude 평가 Capability - Sonnet 모델 사용"""
...
"""
        try:
            response = self.client.messages.create(
                ...
                max_tokens=1024,
                system=system_prompt,
                messages=[{"role": "user", "content": f"제출물:\n{submission}"}]
            )
            result = response.content[0].text
